...
export const getDailyLog = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const dailyLog = await (DailyLog as any).findOrCreate(
      req.user.userId,
      targetDate
    );
...
export const getDailyLogs = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [dailyLogs, total] = await Promise.all([
      DailyLog.find(mongoQuery).sort(sortOptions).skip(skip).limit(limit),
...
export const addDailyLogItem = async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.info(
      `Item added to daily log: ${item.itemName} for user ${req.user.username}`
    );
...
export const updateDailyLogItem = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const itemIndex = dailyLog.items.findIndex(
    ...
    logger.info(
      `Item updated in daily log: ${itemId} for user ${req.user.username}`
    );
...
export const deleteDailyLogItem = async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.info(
      `Item deleted from daily log: ${deletedItem.itemName} for user ${req.user.username}`
    );
...
export const updateWaterIntake = async (req: AuthRequest, res: Response) => {
  try {
    ...
    logger.info(
      `Water intake updated to ${waterIntake} glasses for user ${req.user.username}`
    );
      ...
      data: {
        ...
        totals: {
          avgCalories:
            summaries.length > 0
              ? Math.round(
                  summaries.reduce((sum, s) => sum + s.totalCalories, 0) /
                    summaries.length
                )
              : 0,
          avgProtein:
            summaries.length > 0
              ? Math.round(
                  summaries.reduce((sum, s) => sum + s.totalProtein, 0) /
                    summaries.length
                )
              : 0,
          avgCarbs:
            summaries.length > 0
              ? Math.round(
                  summaries.reduce((sum, s) => sum + s.totalCarbs, 0) /
                    summaries.length
                )
              : 0,
          avgFats:
            summaries.length > 0
              ? Math.round(
                  summaries.reduce((sum, s) => sum + s.totalFats, 0) /
                    summaries.length
                )
              : 0,
          avgWaterIntake:
            summaries.length > 0
              ? Math.round(
                  summaries.reduce((sum, s) => sum + s.waterIntake, 0) /
                    summaries.length
                )
              : 0,
        },
...
export const getAllTimeConsumptionHistory = async (
  req: AuthRequest,
  res: Response
) => {
  try {
    ...
    if (includeTotals === "true") {
      ...
      const firstLogDate = new Date(
        Math.min(...dailyLogs.map((log) => log.date.getTime()))
      );
      const lastLogDate = new Date(
        Math.max(...dailyLogs.map((log) => log.date.getTime()))
      );

      const overallTotals = {
        totalCalories: dailyLogs.reduce(
        ...
        totalWaterIntake: dailyLogs.reduce(
          (sum, log) => sum + log.waterIntake,
          0
        ),
      ...
      const dailyAverages = {
        ...
        avgWaterIntake:
          Math.round((overallTotals.totalWaterIntake / totalDays) * 10) / 10,
        avgItemsPerDay:
          Math.round((overallTotals.totalItems / totalDays) * 10) / 10,
      };
      ...
      dailyLogs.forEach((log) => {
        log.items.forEach((item) => {
          const foodKey = `${item.itemName.toLowerCase().trim()}|${
            item.category
          }`;
          foodFrequency[foodKey] = (foodFrequency[foodKey] || 0) + 1;

          mealFrequency[item.mealType] =
            (mealFrequency[item.mealType] || 0) + 1;
        ...
        .forEach(([month, data]) => {
          monthlyTrends.push({
            ...
            avgCalories: Math.round(
              data.calories.reduce((sum, val) => sum + val, 0) / data.days
            ),
            avgProtein: Math.round(
              data.protein.reduce((sum, val) => sum + val, 0) / data.days
            ),
            totalDays: data.days,
          });
        });

      const yearlyTrends: Array<{
        year: string;
        avgCalories: number;
        avgProtein: number;
        totalDays: number;
      ...
      dailyLogs.forEach((log) => {
        const yearKey = log.date.toISOString().slice(0, 4);
    ...
    logger.info(
      `All-time consumption history retrieved for user ${req.user.username}: ${dailyLogs.length} days`
    );
    ...
    logger.error(
      `Get all-time consumption history error: ${(error as Error).message}`
    );
    res.status(500).json({
      success: false,
      message:
        "Internal server error while fetching all-time consumption history",
    });
  }
};
